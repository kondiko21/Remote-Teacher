...
class SubjectListController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout  {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = "Przedmioty"
        collectionView.register(SubjectCell.self, forCellWithReuseIdentifier: "mainCell")
        ...
        view.addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-0-[v0]-0-|", options: NSLayoutConstraint.FormatOptions(), metrics: nil, views: ["v0": collectionView]))
         ...
         Database.database().reference().child("exercises").observeSingleEvent(of: .value) { (snap) in
        
        for child in snap.children {
            let snap = child as! DataSnapshot
            let value = snap.key
            let ex = value
            self.lessonList.append(ex)
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "mainCell", for: indexPath) as! SubjectCell
        cell.name = lessonList[indexPath.item]
        return cell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let modal = PartListController()
        modal.lesson = lessonList[indexPath.item]
